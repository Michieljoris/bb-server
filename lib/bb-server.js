...
function listen(port, argv) {
    ...
    if (options.sessions && options.sessions && options.sessions.opts && options.sessions.opts.expires) options.sessions.opts.expires *= 60;
    ...
    var server = httpServer.createServer(options)
        .listen(port, host, function() {
            ...
            if (options.authorize)
                out('Authorize: '.grey + getOptionString(options.authorize));
