...
function listen(port, argv) {
    ...
    if (options.sessions.opts.expires) options.sessions.opts.expires *= 60;
    ...
    if (options.reload) {
        ...
        options.recast.inject['index.html'] =
            options.recast.inject['index.html'] || [];
        if (options.recast.inject['index.html'].indexOf('reload') === -1)
            options.recast.inject['index.html'].push('url');
            options.recast.inject['index.html'].push('reload');
    ...
    if (options.cache) {
        ...
        options.cache = {
            ...
            cacheDir : settings.cacheDir || Path.join(process.cwd(), "./cache")
        };
    ...
    var server = httpServer.createServer(options)
        .listen(port, host, function() {
            ...
            if (options.wsHandlers.length)
                out(options.wsHandlers.map(function(h) { return h.id || h }));//
