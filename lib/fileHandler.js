...
function isPrerender(req) {
    ...
    // AnyURL whose query parameters contain the special token
...
function recast() { //fetch function
    ...
    var isModule = file.query && file.query.module;
    // if (parts[parts.length-2] === 'nm') {
    //     parts.splice(parts.length-2, 1);
    //     file.path = parts.join('.');
    //     isModule = true;
    // }
    // console.log('in recast', isModule ? '*module*': '', file);
    var fullPath = Path.join(options.root, file.path);
    recaster.recast({ path: file.path, fullPath: fullPath, encoding: file.encoding , isModule: isModule})
        .when(
            function(data) {
...
// //TODO: get rid of this, always go through recast
...
function getFile(req, res) {
    ...
    if (prerenderKey) {
        ...
        if (mimeType) {
            ...
            fs.stat(filePath, function(err, stat) {
                if (err) {//ERROR in retrieving path..
                    delete pathMap[path];
                    vow.breek({ missing: true, srcPath: path });
                }
                else {
                    if (stat.isDirectory()) {
        ...
        else if (options.index && typeof path !== 'undefined') {
            indexFile.path = Path.join(path, options.index);
            indexFile.encoding = acceptEncoding(req, indexFile.mimeType);
            // indexFile.fetch = (recaster.isRecastable(ext) || indexFile.encoding) ? 
            //     recast : retrieveAsIs;
...
module.exports.get = function(someOptions) {
    ...
    if (options.cache)
        cache = require('./cache')(options.root, options.cacheDir, options.out);
    ...
    authorize = options.authorize || authorize;
