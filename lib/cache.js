            ...
            fs.readJson(Path.resolve(__dirname, cachePath, md5(key)), function(err, value) {
                ...
                validate(value, function(validated) {
                    ...
                    if (validated) {
                        ...
                        fetch(key, function(value, source, maxAge, nocache) {
                            value = {
                                cached: Date.now(),
                                value: value,
                                source: source,
                            ...
                            debug(key + ' ' + 'fetched:', value);
                            
                            if (value.nocache) {
                                memCache.del(key);
                                respond(key, value.value);   
                            }
                            else fs.outputJson(Path.resolve(__dirname, cachePath, md5(key)),
                                               value,
                                               function(err) {
                                                   if (err) {
                                                   ...
                                                   memCache.cache(key, value);   
                                                   debug(key + ' ' + 'responding:', value.value);
                                                   respond(key, value.value);
                                               });
                        });
                    }
