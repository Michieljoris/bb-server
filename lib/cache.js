    ...
    if (vows[key].length === 1) {
        ...
        validate(value, function(validated) {
            if (validated) {
               ...
               else fs.readFile(Path.resolve(__dirname, cachePath, md5(file.stamp + key)), function(err, buff) {
                    var data;
                    if (!err) {
                        ...
                        if (validated) {
                            ...
                            file.fetch().when(
                                function(data) {
                                    ...
                                    fs.outputFile(Path.resolve(__dirname, cachePath, md5(file.stamp + key)),
                                                  buf,
                                                  function(err) {
