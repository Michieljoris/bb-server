...
function trim(str) {
    return str ? str.replace(/^\s+/g, "").replace(/\s+$/g, "") : null;
...
function Sessions(store, opts, storeOpts) {
    ...
    this.store = (typeof store !== "undefined" && store !== null ?
                  new module.exports.stores[store](storeOpts || {}) :
                  new module.exports.stores.memory(storeOpts || {}));
...
Sessions.prototype.create = function () {
    ...
    this.store.add(uid, meta, data, function (err, meta, data) {
	if (err) {
	    return cb(err);
	}
...
Sessions.prototype.checkExpiration = function () {
	...
	this.store.uids(function (err, uids) {
		...
		var i = 0, l = uids.length, missing = l;
...
function buildSessionUtils(uid, meta, data, store, expires, sessions) {
	...
	get: function (key, cb) {
            ...
            if (typeof key === 'function' || cb) store.get(uid, key, cb);
            else {
                var vow = VOW.make();
                // console.log('getting key', uid, key);
                store.get(uid, key, function(err, meta, data) {
                    if (err) vow['break'](err.toString());
                    else {
                        vow.keep({ meta: meta, data: data });
                    }
                });
                return vow.promise;
            }
