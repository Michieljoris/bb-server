...
function getCache(req, path, mimeType, options, cache) {
    ...
    var cacheSettings =
        ...
        (req.remapped ? options.cache._bust_ : undefined);
...
function createHeaders(req, res, mimeType, GMTdate, path) {
    ...
    if (options.cache)
        cache = getCache(req, path, mimeType, options, {
    ...
    if ('*' === accept.trim()) encoding = 'gzip';
        ...
        for (var i = 0, len = zipperMethods.length; i < len; ++i) 
            if (~accept.indexOf(zipperMethods[i])) {
                encoding = zipperMethods[i];
                break;  }
    }
...
//if ever adding more methods, also modifyx
var zipper = {
    gzip: zlib.createGzip
  , deflate: zlib.createDeflate
};

var zipperMethods = Object.keys(zipper);
...
//Send file if modified and gzip it
...
function handleRequest(req, res) {
    ...
    //strip the signature of the filename: 
    ...
    if (!options.trueUrls)  {
        path = pathMap[path];
        if (!path) {
            if (validSignature(baseName.slice(0, signatureLength))) {
                path = baseName.slice(0, signatureLength);
            }
            else path = originalPath;
            pathMap[originalPath] = path;
        }
        if (path !== originalPath) req.remapped = true;
    }
