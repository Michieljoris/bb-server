...
var send, options, pathMap = {}, out, spaFile, indexFile, fetch;
...
function stripStamp(path) {
    ...
    if (options.stamp)  {
        ...
        if (!cached) {
            ...
            if (m && m.index === 0) {
                // extract the hash
                cached.hash = path.slice(prefix.length + 1, prefix.length + 1 + options.stamp.length);
                // 10 first characters of md5 + 1 for slash
                cached.path = path.slice(prefix.length + 1 + options.stamp.length);
            }
            pathMap[path] = cached;
        }
    }
    else cached = { path: path };
    console.log(cached);
    return cached;
}
        ...
        .when(
            function(data) {
                ...
                vow.keep({ value: { status: 200,
                                    ...
                                    body: data.recast },
                           source: filePath,
                           //TODO: oops, mayb not cache stamped files here at the server??
...
function getFile(req, res) {
    ...
    if (prerenderKey) {
        ...
        // //TODO:
        // file.stamp = file.path !== req.url.pathname ? stamp : false;
