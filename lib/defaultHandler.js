...
function createHeaders(req) {
    ...
    if (req.$encoding)
    {  headers['Content-Encoding']= req.$encoding;
    ...
    req.$cacheSettings = cacheSettings;

    headers['Cache-Control'] = cacheSettings.cacheControl;
    if (cacheSettings.expiresIn)
        headers.Expires = new Date(Date.now() + cacheSettings.expiresIn).toString();
...
function recast(req, res, cb) {
    ...
    //produce a file path:
    var srcPath = Path.join(options.root, req.$path);
    recaster.recast({ srcPath: srcPath,
        ...
        .when(
            function(data) {
                cb({ value: { status: 200,
                     ...
                     source:srcPath,
                     nocache: false,
                     maxAge: req.$cacheSettings.expiresIn
                   });
                
            } ,function(data) {
                //no luck at recasting, sending the original srcData
                //found at srcPath 
                if (data.srcData) {
                    cb({ value: { status: 200,
                                  headers: createHeaders(req),
...
function prerender(req, res, cb) {
    ///TODO: fix this and recaster should zip up on error but srcData is ok.
...
function getKey(req, res) {
    ...
    if (prerenderKey) {
        ...
        var key = stripStamp(req.url.pathname);
        req.$stamped = key !== req.url.pathname;
        req.$mimeType = mime.lookup(Path.extname(key));
        if (req.$mimeType) {
            vow.keep({ key: key, fetch: recast });
        }
        else if (options.spa) {
            if (req.$fromBot) {
                vow.keep({ key: key, fetch: prerender });
            }
            else {
        ...
        //no spa:;
        else if (listableDir(key)) {
            fs.stat(key, function(err, stat) {
                if (err) {
                    delete pathMap[key];
                    sendMisc.missing(req, res, key);
                    vow.breek();
                }
                else {
                    if (stat.isDirectory()) { 
                        send.dir(req, res, key, function(key) {
                            //path is autoindex
                            vow.keep({ key: key, fetch: recast });
...
function handleRequest(req, res) {
    ...
    req.$path = Path.resolve('/', req.url.pathname);
    getKey(req, res).when(
        function(key) {
            //key.path is the actual relative path to the resource
            //requested, so without the stamp and redirected if a spa.
            req.$encoding = acceptEncoding(req);
            req.$key = key.key + '.' + req.$encoding;
            //modify key if gzip or deflate is required
            cache(req.$key, bind(key.fetch, req, res), bind(sendValue, req, res));
        }
        ,function() {
