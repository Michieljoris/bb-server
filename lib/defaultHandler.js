...
function createHeaders(src) {
    ...
    src.cacheSettings = (function() {
        if (options.cache) {
            if (src.prerender) return options.cachePrerender;
            else if (src.stamped) return options.cacheStamped;
    ...
    headers['Last-Modified'] = Date.now(); //src.GMTdate.toString();
...
function prerender() {
    var vow = VOW.make();
    var src = this;
    wash(src.path, { phantomPath: options.phantomPath,
                     seoServer: options.seoServer })
        .when(
            function(result) {
...
function source(req, res) {
    ...
    if (prerenderKey) {
        vow.keep({ path: prerenderKey, prerender: true, fetch: prerender });
    }
    else {
        ...
        if (mimeType)
            ...
            if (req.$fromBot)
                vow.keep({ path: path, prerender: true, fetch: prerender });
        ...
        //no spa:;
            ...
            fs.stat(path, function(err, stat) {
                if (err) {
                    ...
                    if (stat.isDirectory()) { 
                        send.dir(req, res, path, function(path) {
...
function handleRequest(req, res) {
    ...
    req.url = Url.parse(Url.format(req.url));
    source(req, res).when(
        function(src) {
            if (src)  {
                ...
                if (options.cache)  {
                    ...
                    return cache(key, function() { return src.fetch(); });
                }
            ...
            function(data) {
                if (data) { 
                    if (!dev && Date.parse(req.headers["if-modified-since"]) < data.cached)  {
                ...
                debug('Finished request for:', req.url.href);
            } 
            ,function(err) {
