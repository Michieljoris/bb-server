...
function matchRoute(routes, path) {
    ...
    return r ? routes[r] : false;
}

// console.log(matchRoute({ "^path/to(/.*)?$": 1, "^b$":2 }, 'path/to'));
        ...
        'GET': Object.keys(options.getHandlers).length ?
            function(req, res) {
                ...
                (apiHandler('GET', path) ||
                 ...
                 matchRoute(options.getHandlers, path) ||
                 fileHandler)(req, res, fileHandler, send);
            } : fileHandler,
        'POST': Object.keys(options.postHandlers).length ?
            function (req, res) {
                var path = req.url.pathname;
                (apiHandler('POST', path) ||
                 // options.postHandlers[path] ||
                 matchRoute(options.postHandlers, path) ||
                 function() { send.forbidden(req, res, req.url.path); })(req, res, send);
            }: function() {},
        'HEAD': fileHandler
